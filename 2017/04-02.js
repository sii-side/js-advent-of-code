...
function solve (input) {
  const passPhrases = new PassPhrases(input.trim().split('\n'))
  return passPhrases.validPhrases().length
}

class PassPhrases {
  constructor (passPhraseArray) {
    this.all = passPhraseArray.map(passPhrase => new PassPhrase(passPhrase))
  }

  validPhrases () {
    return this.all.filter(passPhrase => passPhrase.validate())
  }
}
...
class PassPhrase {
  constructor (value) {
    this.value = value
    this.words = new Words(this.value.split(/[\u0020]+/))
  }

  validate () {
    return this.words.validate()
  }
}

class Words {
  constructor (words) {
    this.all = words.map(word => new Word(word))
  }

  validate () {
    return this.all.every((word, index) => {
      return this.exclude(index).every(targetWord => word.sort() !== targetWord.sort())
    })
  }

  exclude (index) {
    const prevAll = this.all.slice(0, index)
    const nextAll = this.all.slice(index + 1)
...
class Word {
  ...
  sort () {
    return Array.from(this.value).sort().join('')
  }
}
